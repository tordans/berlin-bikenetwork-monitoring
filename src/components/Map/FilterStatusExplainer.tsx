import { twJoin } from 'tailwind-merge'
import { useAppActions, useShowExplainer, useStatus } from '../store'
import { statusFilters } from './data/statusFilters.const'

export const FilterStatusExplainer = () => {
  const status = useStatus()
  const showExplainer = useShowExplainer()
  const { toggleShowExplainer } = useAppActions()
  const explainer = statusFilters.find((s) => s.key == status)?.explainer
  if (!explainer) return null

  return (
    <>
      <button
        className={twJoin(
          'text-ccOrange-600 hover:bg-ccGray-100 absolute top-0 right-0 flex h-6 w-6 items-center justify-center rounded-full bg-white text-xs font-semibold hover:shadow',
        )}
        onClick={toggleShowExplainer}
      >
        {showExplainer ? (
          <svg
            // Close X
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            className="h-4 w-4"
          >
            <path
              fillRule="evenodd"
              d="M8 15A7 7 0 1 0 8 1a7 7 0 0 0 0 14Zm2.78-4.22a.75.75 0 0 1-1.06 0L8 9.06l-1.72 1.72a.75.75 0 1 1-1.06-1.06L6.94 8 5.22 6.28a.75.75 0 0 1 1.06-1.06L8 6.94l1.72-1.72a.75.75 0 1 1 1.06 1.06L9.06 8l1.72 1.72a.75.75 0 0 1 0 1.06Z"
              clipRule="evenodd"
            />
          </svg>
        ) : (
          <svg
            // Question mark
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 16 16"
            fill="currentColor"
            className="h-4 w-4"
          >
            <path
              fillRule="evenodd"
              d="M15 8A7 7 0 1 1 1 8a7 7 0 0 1 14 0Zm-6 3.5a1 1 0 1 1-2 0 1 1 0 0 1 2 0ZM7.293 5.293a1 1 0 1 1 .99 1.667c-.459.134-1.033.566-1.033 1.29v.25a.75.75 0 1 0 1.5 0v-.115a2.5 2.5 0 1 0-2.518-4.153.75.75 0 1 0 1.061 1.06Z"
              clipRule="evenodd"
            />
          </svg>
        )}
        <span className="sr-only">Erklärung anzeigen</span>
      </button>
      {showExplainer ? (
        <p className="mt-2 mr-8 max-w-[30rem] text-sm leading-tight">{explainer}</p>
      ) : (
        ''
      )}
    </>
  )
}
